...
class SafetyTests(unittest.TestCase):
 ...
 def test_natural_log_requires_success_and_all_safety_markers(self):
  log='TELEGRAM_OUTPUT_LAYER {"enabled":true,"info_enabled":true,"final_chain_auto":false}\nV3_TELEGRAM_DELIVERY_SIDECAR {"network_send":false}\n{"ok":true}'
  self.assertTrue(owner.verify_natural_log(log))
  for token in ['"enabled":true','"info_enabled":true','"final_chain_auto":false','"network_send":false','"ok":true']:
 ...
 def test_natural_budget_block_summary_is_not_a_false_veto(self):
  log='V3_TELEGRAM_DELIVERY_SIDECAR {"network_send":false}\n2026-09-20T17:04:00Z {"ok":true,"telegram_output":{"version":"telegram-output-budget-guard","enabled":true,"info_enabled":true,"final_chain_auto":false,"morning":{"status":"BLOCKED_D1_PREACTION_BUDGET"}}}'
  self.assertTrue(owner.verify_natural_log(log))
  with self.assertRaises(owner.SafetyFailure):owner.verify_natural_log(log.replace('"final_chain_auto":false','"final_chain_auto":true'))
  split='V3_TELEGRAM_DELIVERY_SIDECAR {"network_send":false}\n{"ok":true,"telegram_output":{"enabled":true}}\n{"info_enabled":true,"final_chain_auto":false}'
  with self.assertRaises(owner.SafetyFailure):owner.verify_natural_log(split)
  contradictory=log.replace('"enabled":true','"enabled":true,"enabled":false')
  with self.assertRaises(owner.SafetyFailure):owner.verify_natural_log(contradictory)
  with self.assertRaises(owner.SafetyFailure):owner.verify_natural_log(log.replace('"network_send":false','"network_send":false,"network_send":true'))
 ...
 def test_ambiguous_dispatch_posts_only_once(self):
  ...
  flat=calls[0][0]
  self.assertIn('isolated_telegram_validation=true',flat)
