...
test('one-off index migration has a cardinality-derived bounded D1 reservation',()=>{
 const s=fs.readFileSync(new URL('../tools/d1-preflight.mjs',import.meta.url),'utf8');
 assert.match(s,/LIMIT 2049/);assert.match(s,/cardinality\.length>2048/);
 assert.match(s,/rows_read:8192,rows_written:2304/);
 assert.doesNotMatch(s,/rows_written:4096/);
});
