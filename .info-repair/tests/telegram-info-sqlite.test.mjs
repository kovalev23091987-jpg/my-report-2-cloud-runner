...
import { runTelegramOutputLayer, reserveDispatch, buildBudgetBlockedTelegramOutput, buildMorningInformationalMessage, buildWaitInformationalMessage } from '../candidate/runner/telegram-output.mjs';
import { reserveInformational, normalizeInfoRow, loadInformationalRows, INFO_D1_BUDGET, INFO_LIMITS } from '../candidate/runner/telegram-info-runtime.mjs';
...
test('budget-block proof preserves actual gates and bounded envelope fits the run headroom observed remotely',()=>{
 ...
 assert.deepEqual(INFO_D1_BUDGET,{rowsRead:1536,rowsWritten:4});assert.ok(13538+INFO_D1_BUDGET.rowsRead<=16000);assert.ok(156+INFO_D1_BUDGET.rowsWritten<=320);
});
