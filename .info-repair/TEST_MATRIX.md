...
| Owner safety | proof identity, safety flags, one-shot dispatch, rollback ownership, natural logs, D1 budget-block summary | PASS |
...
| R6 production rollback | два natural success; ложный veto на пропущенной standalone-строке; exact-base tree восстановлен | PASS / DIAGNOSED |

Локальные PASS относятся к байтам пакета, зафиксированным в `MANIFEST.json` и `SHA256SUMS`. R7 ещё не запускался удалённо: это выполнит команда владельца до нового изменения `main`.
