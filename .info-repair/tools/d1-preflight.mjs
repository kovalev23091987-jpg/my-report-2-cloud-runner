...
async function main() {
  ...
  if(mode==='--apply-index'&&!index.length) {
    ...
    // The write reservation stays at the production burst size. Reserving 4096
    // writes for one additive index would falsely consume the daily admission
    // budget even though the operation is bounded and read back immediately.
    const cap={rows_read:8192,rows_written:320};
