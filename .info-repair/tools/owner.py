...
BASE='06456805eded133965324b1e5786daf1ad783575'
...
def verify_natural_log(text):
    ...
    output_tokens=['"enabled":true','"info_enabled":true','"final_chain_auto":false']
    unsafe_output_tokens=['"enabled":false','"info_enabled":false','"final_chain_auto":true']
    standalone_output=any('TELEGRAM_OUTPUT_LAYER' in line and all(token in line for token in output_tokens) and not any(token in line for token in unsafe_output_tokens) for line in lines)
    # A D1 preaction budget block intentionally skips the standalone output
    # line. The final runner result remains authoritative and contains the
    # exact fail-closed telegram_output state on the same JSON log line.
    summary_output=any('"ok":true' in line and '"telegram_output":{' in line and all(token in line[line.index('"telegram_output":{'):line.index('"telegram_output":{')+512] for token in output_tokens) and not any(token in line[line.index('"telegram_output":{'):line.index('"telegram_output":{')+512] for token in unsafe_output_tokens) for line in lines)
    delivery_safe=any('V3_TELEGRAM_DELIVERY_SIDECAR' in line and '"network_send":false' in line and '"network_send":true' not in line for line in lines)
    runner_ok=any('"ok":true' in line for line in lines)
    if not (standalone_output or summary_output) or not delivery_safe or not runner_ok:raise SafetyFailure('NATURAL_LOG_SAFETY_MARKER_MISSING')
    forbidden=['INFO_ERROR_FAIL_CLOSED','INFO_ACK_UNKNOWN','INFO_FINALIZE_ACK_UNKNOWN','INFO_RESERVATION_READBACK_FAILED','INFO_FINALIZE_READBACK_FAILED','INFO_CORRUPT_JOURNAL_STATE','INFO_IDENTITY_COLLISION','INFO_FINAL_AUTO_CONFLICT']
    if any(token in text for token in forbidden):raise SafetyFailure('NATURAL_LOG_INFORMATIONAL_FAILURE')
    return True
    ...
    steps:
      ...
      - name: Exact isolation and payload
        ...
        run: |
          ...
          test "$(git rev-parse origin/main)" = 06456805eded133965324b1e5786daf1ad783575
          test "$(git rev-parse HEAD^)" = 06456805eded133965324b1e5786daf1ad783575
...
def self_test():
    ...
    for name in ['owner-safety.test.py','package-static.test.py']:
        r=subprocess.run([sys.executable,str(ROOT/'tests'/name)],text=True,capture_output=True)
        if r.returncode:raise SafetyFailure('LOCAL_TEST_FAIL:'+name+'\n'+r.stderr[-1000:])
    ...
    if sums.exists():
        ...
        actual={str(p.relative_to(ROOT)):sha(p) for p in ROOT.rglob('*') if p.is_file() and p!=sums and '__pycache__' not in p.parts}
        if set(expected)!=set(actual):raise SafetyFailure('PACKAGE_FILESET_MISMATCH')
        for name,digest in expected.items():
            if actual[name]!=digest:raise SafetyFailure('PACKAGE_FULL_CHECKSUM:'+name)
...
class Installer:
    ...
    def dispatch(self,branch,head,label,mode):
        ...
        self.cmd(['gh','workflow','run','report2.yml','-R',REPO,'--ref',branch,'-f','reason='+label+' '+self.tag,'-f',field,'-f','isolated_telegram_validation=true'],check=False)
    ...
    def install(self):
        ...
        shutil.copytree(ROOT,self.repo/'.info-repair',ignore=shutil.ignore_patterns('evidence','SHA256SUMS','__pycache__','*.zip','*.command'))
        # Evidence logs remain in the signed ZIP. They are deliberately not put
        # through Git because Git may normalize archived CRLF output. Remote
        # self-test hashes only functional sources, fixtures and tests.
