...
// The existing production journal is reused without schema changes.
// Keep the already-proven category and namespace informational rows via dispatch_key/source_ref.
const INFO_JOURNAL_CATEGORY = "FINAL_CHAIN_CANDIDATE";
...
async function recentInfoWaitCooldown(db,contract,direction,now) {
  const ref=`INFO_WAIT|${String(contract||"")}|${infoDirection(direction)||"UNKNOWN"}`;
  const row=await db.prepare(`SELECT status,reserved_ts FROM telegram_output_dispatch_journal_v2
    WHERE category='FINAL_CHAIN_CANDIDATE' AND source_ref=?1 AND reserved_ts>=?2
...
async function sendInformationalTelegram({db,rows,now,source,relayUrl,relayKey,fetchImpl,reportTest=false}={}) {
  ...
  if(morningDue){
    ...
    if(!built.ok){out.morning={status:built.status,sent:false};}
      ...
      const key=forceTest?`final-chain:info-morning-test:${Math.trunc(Number(now))}`:`final-chain:info-morning:${clock.date}`;
      const sourceRef=forceTest?`INFO_MORNING_TEST|${clock.date}|${Math.trunc(Number(now))}`:`INFO_MORNING|${clock.date}`;
      const res=await reserveDispatch(db,{dispatchKey:key,category:INFO_JOURNAL_CATEGORY,sourceRef,text:built.message,now});
  ...
  if(!morningDue && String(source||"")==="schedule"){
    ...
    if(wait){
      ...
      if(!built.ok) out.early_info={status:built.status,sent:false,count:0};
        ...
        const key=`final-chain:info-wait:${String(wait.contract)}:${infoDirection(wait.direction)}:${String(wait.wave_id||"")}`;
        const sourceRef=`INFO_WAIT|${String(wait.contract)}|${infoDirection(wait.direction)}`;
        const res=await reserveDispatch(db,{dispatchKey:key,category:INFO_JOURNAL_CATEGORY,sourceRef,text:built.message,now});
