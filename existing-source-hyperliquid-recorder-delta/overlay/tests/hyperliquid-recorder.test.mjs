...
test('clearinghouseState distinguishes subsequent position state without global inference',async()=>{
  ...
  const isolatedAddress='0x0000000000000000000000000000000000000001';
  const a=await mod.fetchExistingSmartMoneyRecorderRaw({fetch_impl:f,bykaranteli_fetcher:null,contract_code:'BTC-USDT',hyperliquid_sample_address:isolatedAddress,observed_ts:1790000000000,force_method:'clearinghouseState'});
  const b=await mod.fetchExistingSmartMoneyRecorderRaw({fetch_impl:f,bykaranteli_fetcher:null,contract_code:'BTC-USDT',hyperliquid_sample_address:isolatedAddress,observed_ts:1790000300000,force_method:'clearinghouseState'});
