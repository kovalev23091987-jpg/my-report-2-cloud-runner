...
function enumeratePaths(value, path = [], output = []) {
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < Math.min(value.length, 5); index += 1) {
      enumeratePaths(value[index], [...path, index], output);
    }
  } else {
  ...
  for (const open of [false, true]) {
    ...
    for (const path of enumeratePaths(source)) {
      ...
      for (const kind of kinds) {
        ...
        let output;
        assert.doesNotThrow(() => {
          output = buildFinalDecisionIntegrationShadow(mutatedCopy(source, path, kind));
        }, `${direction}:${open ? "OPEN" : "FLAT"}:${path.join(".")}:${kind}`);
        assert.deepEqual(
          validateFinalDecisionOutput(output),
          { valid: true, errors: [] },
          `${direction}:${open ? "OPEN" : "FLAT"}:${path.join(".")}:${kind}`,
        );
...
assert.ok(cases >= 9_000, `mutation coverage unexpectedly shrank: ${cases}`);
console.log(JSON.stringify({
  ok: true,
  suite: "final-decision-integration-builder-totality",
  mutation_cases: cases,
  assertions: "every one-field deletion/null/type/future mutation returns a validator-safe shadow-only envelope",
}));
