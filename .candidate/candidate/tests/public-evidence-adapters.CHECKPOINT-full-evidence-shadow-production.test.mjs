...
} from "../src/public-evidence-adapters.CHECKPOINT-full-evidence-shadow-production.mjs";
...
assert.ok(ascii.evidence.some(x=>x.metric==="funding_rate" && x.venue==="BYBIT" && x.status==="SOURCE_INCOMPATIBLE" && x.venue_observation_status==="CLOSED"));
assert.ok(ascii.evidence.filter(x=>x.alias_required).every(x=>x.alias_verification_scope==="VENUE_MARKET_SYMBOL_ONLY" && x.asset_identity_verified===false));
assert.ok(ascii.evidence.some(x=>x.metric==="rs_vs_btc_4h" && x.status==="SOURCE_INCOMPATIBLE" && x.venue_observation_status==="CLOSED"));
