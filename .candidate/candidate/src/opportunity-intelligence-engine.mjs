  ...
  const events = selectBoundedFairEvents(independentEpisodes, config.max_events_per_analysis)
    .map((event) => {
      ...
      return {
        ...
        drilldown: {
          one_minute_bars: oneMinute.candles.filter((row) => row.ts >= event.timestamp && row.ts < event.event_close_ts).length,
          five_minute_bars: series["5m"].filter((row) => row.ts >= event.timestamp && row.ts < event.event_close_ts).length,
        },
      };
    });
