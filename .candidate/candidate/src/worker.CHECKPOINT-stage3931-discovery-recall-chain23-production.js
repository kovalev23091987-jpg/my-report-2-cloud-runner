...
const EXTERNAL_REQUEST_RESERVE = 7;
...
function buildDiscoveryPrefilter(
  ...
  const fundingPerHourOf =
    (row) => {
       ...
       * raw funding for the legacy neutral
       * absolute-extreme flag, but it cannot
       * create a directional funding-tail
       * recall route.
       */
      return null;
    };
  ...
  return {
    ...
    rules: [
      ...
      "Technical eligibility remains fail-closed: CLOSED, fresh, exact HTX identity, OI, funding, history and confirmed crypto scope are still required.",
      ...
      "Funding directional tails require a factual funding interval; missing interval never creates a directional funding route.",
      ...
      "Existing Fast-Move Watch fairness/cooldown and the one-Deep-Check-per-cycle cap remain authoritative.",
    ],
  };
}
...
function buildBoundedDeepCheckPlan(
   ...
   * cross-venue + 2 HTX liquidation + 4 projected-map provider = 39.
   * Stage-0 uses 4. A 7-call reserve remains below the known limit.
   */
...
const buildShadowDecisionTelemetry = (() => {
  const RULES_VERSION = "shadow-dc-eq-dq-v1.2-execution-closure";
...
const collectPublicFullEvidence = (() => {
  ...
  function buildCrossVenueAssetIdentityProof(contractCode, verification = {}) {
    ...
    } else if (okxSpot && (bybit || binance)) {
      verified = true;
      method = "HTX_CANONICAL_PLUS_OKX_SPOT_PLUS_EXTERNAL_PERP_EXACT_BASE";
    } else if (bybit && binance) {
      verified = true;
      method = "HTX_CANONICAL_PLUS_BYBIT_BINANCE_EXACT_BASE";
    }
  ...
  async function collectPublicFullEvidence({ contract_code, fetch_impl, now_ts = Date.now() } = {}) {
    ...
    const rawEvidence = [...bybit, ...okx, ...binance, ...rs.evidence, ...externalRequired];
    ...
    Object.defineProperty(response, "_opportunity_hourly_candles", {
      ...
      value: {
        ...
        BINANCE_PERP: binance._opportunity_hourly_candles || [],
...
async function buildDeepCheckInput(params, env) {
  ...
  const preparedFullEvidenceProof =
    prepareFullEvidenceProofBundle({
      ...
      // Do not translate legacy weighted/numeric shadow heuristics into
      // independent decision evidence. Until a factual causal-domain producer
      // exists, the registry is intentionally empty and Final Decision may WAIT
      // or fail closed.
      decision_evidence: [],
      committed_ts: now,
    });
  ...
  if (sealedFullEvidenceProof?.status === "CLOSED" && sealedFullEvidenceProof?.bundle) {
...
return {
    ...
    stage392_shadow_integration: {
      version: "stage392-shadow-integration-v2",
      mode: "SHADOW_ONLY_NO_EXECUTION",
      proof_status:
