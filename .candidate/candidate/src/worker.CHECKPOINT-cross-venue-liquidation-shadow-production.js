...
async function persistStage0(
  ...
  const output = {
    ...
    version:
      "3.7.1-cross-venue-liquidation-shadow",
...
async function persistFullEvidenceShadowRecord(env, record) {
  if (!env?.DATA_DB) {
    return {
      status: "SOURCE_UNSUPPORTED",
      persisted: false,
  ...
  const now = Date.now();
  const retentionCutoff = now - 180 * 24 * 60 * 60 * 1000;
  ...
  try {
    ...
    const results = await env.DATA_DB.batch([insert, cleanup]);
    return {
      status: "CLOSED",
      persisted: true,
      insert_changes: Number(results?.[0]?.meta?.changes ?? 0),
      cleanup_changes: Number(results?.[1]?.meta?.changes ?? 0),
      error: null,
    };
  } catch (error) {
    return {
...
const LIQUIDATION_INTELLIGENCE_API = (() => {
  ...
  function explicitVenues(payload) {
    ...
    for (const v of candidates) {
      if (Array.isArray(v)) {
        const xs = [...new Set(v.map(x => txt(typeof x === "object" ? (x?.venue || x?.exchange || x?.name || x?.id) : x)).filter(Boolean))];
        if (xs.length) return xs;
      }
      if (typeof v === "string") {
        const xs = [...new Set(v.split(/[,;+|]/).map(txt).filter(Boolean))];
        if (xs.length) return xs;
      }
    }
    return [];
  ...
  function projectedContainerEntries(payload) {
    ...
    function walk(node, path = "root", depth = 0) {
      ...
      if (Array.isArray(node)) {
        ...
        for (let i = 0; i < Math.min(node.length, 5); i++) walk(node[i], `${path}[${i}]`, depth + 1);
        return;
      }
  ...
  function parseProviderSymbolRegistry(payload, expectedSymbol) {
    ...
    const rows = Array.isArray(payload?.symbols) ? payload.symbols : [];
    const exact = rows.find((row) => txt(row?.symbol).toUpperCase() === target) || null;
    return {
      source_ts: sourceTs,
  ...
  function parseProjectedMap(payload, { observedTs, expectedSymbol } = {}) {
    ...
    const rows = [];
    for (const c of projectedContainerEntries(payload)) {
    ...
    const clusters = dedupeClusters(rows);
    ...
    return { source_ts: sourceTs, current_price: currentPrice, venues_covered: venues, clusters, response_symbol: responseSymbol || null, response_symbol_match: symbolMatch };
  }

  function findSymbolRows(node, symbol, out = [], depth = 0, path = "root") {
    if (depth > 6 || node === null || node === undefined) return out;
    if (Array.isArray(node)) {
      for (let i = 0; i < node.length; i++) findSymbolRows(node[i], symbol, out, depth + 1, `${path}[${i}]`);
      return out;
    }
    ...
    if (sym && target && (sym === target || sym === `${target}USDT` || sym.replace(/[-_/]/g, "") === `${target}USDT`)) {
      out.push({ path, row: node });
    }
    for (const [k, v] of Object.entries(node)) {
      if (typeof v === "object" && v !== null) findSymbolRows(v, symbol, out, depth + 1, `${path}.${k}`);
    }
    return out;
  }
  ...
  function parseRealizedSummary(payload, expectedSymbol, observedTs) {
    const sourceTs = rootSourceTimestamp(payload);
    const rows = findSymbolRows(payload, expectedSymbol);
    const compact = [];
    ...
    return { source_ts: sourceTs, rows: compact };
  }
  ...
  function parseCoverage(payload, observedTs) {
    ...
    return { evidence_type: "MULTI_VENUE_MODEL_COVERAGE", provider: PROVIDER, source_ts: sourceTs, observed_ts: observedTs, venues };
  }
  ...
  async function collectCrossVenueLiquidationIntelligence({ contract_code, fetch_impl, api_key, now_ts = Date.now(), htx_liquidation_tape = null } = {}) {
    ...
    let projected = { source_ts: null, current_price: null, venues_covered: [], clusters: [], response_symbol: null, response_symbol_match: null };
    ...
    const symbolRegistry = symbolsRaw.ok
      ...
      : { source_ts: null, target_symbol: `${alias.provider_symbol}USDT`, exact_match: false, matched_symbol: null, registry_size: 0 };
    ...
    const realizedParsed = realizedRaw.ok ? parseRealizedSummary(realizedRaw.data, alias.provider_symbol, nowTs) : { source_ts: null, rows: [] };
    ...
    if (realizedRaw.ok && realizedParsed.rows.length && realizedFresh.status === "CURRENT") realizedStatus = "OBSERVATION_ONLY_IDENTITY_UNVERIFIED";
    ...
    else if (["SOURCE_UNSUPPORTED", "SOURCE_INCOMPATIBLE", "AUTH_ERROR", "SOURCE_ERROR", "STALE", "FUTURE", "MISSING_SOURCE_TIMESTAMP"].includes(projectedStatus)) dq = projectedStatus;
  ...
  async function persistShadow(env, record, now = Date.now()) {
    ...
    try {
      ...
      for (const c of (lifecycleStateEligible && Array.isArray(record?.projected_clusters) ? record.projected_clusters : []).slice(0, 80)) {
        if (!c?.cluster_key || finite(c?.level_price) === null) continue;
        stateStatements.push(env.DATA_DB.prepare(`
        ...
        `).bind(
          c.cluster_key, record?.provider || PROVIDER, contract, c.side || "UNKNOWN", c.level_price,
          c.price_low ?? null, c.price_high ?? null, c.source_unit ?? null, observedTs, observedTs,
          c.lifecycle || "ACTIVE", c.distance_pct ?? null, c.explicit_major ? 1 : 0,
          c.raw_size ?? null, c.normalized_strength ?? null, RULES_VERSION, observationId
        ));
      }
      ...
      const results = await env.DATA_DB.batch([insert, ...stateStatements, cleanupObs, expireState, deleteState]);
      return {
        status: "CLOSED",
        persisted: true,
        insert_changes: Number(results?.[0]?.meta?.changes ?? 0),
        state_rows_attempted: stateStatements.length,
        error: null,
      };
    } catch (error) {
