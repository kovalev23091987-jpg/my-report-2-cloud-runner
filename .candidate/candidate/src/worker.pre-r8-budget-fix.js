...
const DEEP_CHECK_EXTERNAL_REQUESTS = 40;
...
function buildBoundedDeepCheckPlan(
   ...
   * cross-venue + 2 HTX liquidation + 4 projected-map provider = 39, plus
   * one bounded raw Smart Money observation = 40. Stage-0 uses 4. A 6-call
   * reserve keeps the same known total external-request envelope of 50.
   */
...
async function buildDeepCheckInput(params, env) {
  ...
  if (publicEvidence && Array.isArray(publicEvidence.evidence)) {
    publicEvidence.evidence.push(...smartMoneyRawEvidenceRows(smartMoneyRaw, now));
  }
  ...
  // TZ 10.1 publication stays separate from analytical Final Decision. The
  // current implementation intentionally has no scenario/cost producer yet,
  // so it reports exact missing gates and spends zero sidecar D1 statements.
