...
export function produceTz101HardVeto({
  ...
  // Hard-veto owns only the factual SAFETY blocker family. Direction, scenario,
  // invalidation/risk, timing and execution are separate Final Decision gates;
  // publication additionally requires score/entry/target/all-cost context.
  // Duplicating those gates here would make a clean safety layer permanently
  // UNKNOWN and create a circular admission dependency.
  const status = producerOwnedStatus;
  const authoritative = producerOwnedStatus !== 'UNKNOWN';
  ...
  return {
    ...
    unknown_reasons: uniq(unknownChecks.map((c) => c.reason)),
    ...
    semantics: status === 'VETO'
      ...
      : status === 'CLEAR'
        ? 'AUTHORITATIVE_SAFETY_VETO_SCOPE_CLEAR_OTHER_DECISION_AND_PUBLICATION_GATES_REMAIN_SEPARATE'
