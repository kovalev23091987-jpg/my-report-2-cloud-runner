...
const OUTPUT_VERSION = "telegram-output-v7-exact-decision-context";
...
export function extractExactScoreContext(observer, threshold = 70, now = Date.now()) {
  if (!observer) return { ok:false, status:"NO_FINAL_DECISION", context:null };
  const d = observer.decision_json_object || parseJsonObject(observer.decision_json);
  const c = d?.telegram_context_v1;
  if (!c || typeof c !== "object" || Array.isArray(c)) return { ok:false, status:"EXACT_SCORE_CONTEXT_NOT_AVAILABLE", context:null };
  if (c.schema !== EXACT_SCORE_SCHEMA || c.score_semantics !== EXACT_SCORE_SEMANTICS || c.is_probability !== false) {
  ...
  const bindings = [
    [String(c.decision_id || "") === String(observer.decision_id || ""), "SCORE_DECISION_ID_MISMATCH"],
    [String(c.material_digest || "") === String(observer.material_digest || ""), "SCORE_MATERIAL_DIGEST_MISMATCH"],
    [String(c.snapshot_id || "") === String(observer.snapshot_id || ""), "SCORE_SNAPSHOT_ID_MISMATCH"],
    [Number(c.observation_ts || 0) === Number(observer.observation_ts || 0), "SCORE_OBSERVATION_TS_MISMATCH"],
    [String(c.direction || "").toUpperCase() === direction, "SCORE_DIRECTION_MISMATCH"],
    [String(c.decision_evidence_receipt_id || "") === String(observer.decision_evidence_receipt_id || ""), "SCORE_DECISION_EVIDENCE_RECEIPT_MISMATCH"],
    [String(c.full_evidence_receipt_id || "") === String(observer.full_evidence_receipt_id || ""), "SCORE_FULL_EVIDENCE_RECEIPT_MISMATCH"],
    [String(c.safety_gate_receipt_id || "") === String(observer.safety_gate_receipt_id || ""), "SCORE_SAFETY_GATE_RECEIPT_MISMATCH"],
  ];
  for (const [ok, status] of bindings) if (!ok) return { ok:false, status, context:null };

  // The existing shadow ledger is an internal analytical episode. No portfolio
  // connector is authoritative here, so user position must remain UNKNOWN and
  // an alert must never be interpreted as proof that the user traded.
  ...
  if (!validUntil || validUntil < tsNow || validUntil > Number(observer.observation_ts || 0) + FINAL_DECISION_FRESH_MS) {
...
export async function runTelegramOutputLayer({
  ...
  try {
    ...
    for (const observer of finalRows) {
      ...
      const extracted=extractExactScoreContext(observer,threshold,ts);
