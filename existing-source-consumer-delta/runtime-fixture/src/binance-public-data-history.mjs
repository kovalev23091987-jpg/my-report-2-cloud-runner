...
export const BINANCE_PUBLIC_DATA_HISTORY_VERSION='binance-public-data-history-v2-20260925';
...
export function buildBinanceArchiveUrl({market='spot',frequency='daily',data_type='klines',symbol,interval='1m',date}={}){
  ...
  // Official Binance naming for klines omits the literal "klines" token:
  // SYMBOL-INTERVAL-DATE.zip. trades/aggTrades keep their data-type token.
  const file=type==='klines'
    ?`${sym}-${interval}-${suffix}.zip`
    :`${sym}-${type}-${suffix}.zip`;
