...
const W_HELPER_OLD=`const EXTERNAL_REQUEST_RESERVE = 6;

const JSON_HEADERS = {`;
const W_HELPER_NEW=`const EXTERNAL_REQUEST_RESERVE = 6;
...
if(already){
 const proof={version:'snapshot-diff-overlay-v1-20260925',status:'ALREADY_APPLIED',base_candidate_sha:'dc38d18cf1fd9cba321f2b9cf0830ed39d136537',production_base:'08d98579c5ecbeb4de426ffeb1160f25ece52708',worker_after_sha256:sha(worker),canonical_after_sha256:sha(canonical),module_sha256:sha(modTarget),production_changed:false,d1_migration:false,d1_write_delta:0,d1_read_delta_per_deep_check_max:1,worker_relative_import_added:false,telegram_send:false,telegram_recipients_changed:false,cloudflare_deploy:false,force_push:false,hard_gates_changed:false,strategy_weights_changed:false,thresholds_changed:false,automatic_execution:false,validated_signal:false,live_probability:false,new_sources_added:false,worker_copy_added:false,hot_cycle_external_request_delta:0};
...
for(const [label,s,anchor] of [['worker helper',w,W_HELPER_OLD],['worker context',w,W_CONTEXT_OLD],['worker pass',w,W_PASS_OLD],['canonical version',c,C_VERSION_OLD],['canonical import',c,C_IMPORT_OLD],['canonical signature',c,C_SIG_OLD],['canonical support',c,C_SUPPORT_OLD],['canonical metadata',c,C_META_OLD]])if(!s.includes(anchor))throw new Error(`SNAPSHOT_DIFF_PATCH_ANCHOR_MISSING:${label}`);
w=w.replace(W_HELPER_OLD,W_HELPER_NEW).replace(W_CONTEXT_OLD,W_CONTEXT_NEW).replace(W_PASS_OLD,W_PASS_NEW);
...
try{
 ...
 const proof={version:'snapshot-diff-overlay-v1-20260925',status:'APPLIED',base_candidate_sha:'dc38d18cf1fd9cba321f2b9cf0830ed39d136537',production_base:'08d98579c5ecbeb4de426ffeb1160f25ece52708',worker_before_sha256:BEFORE_WORKER,worker_after_sha256:sha(worker),canonical_before_sha256:BEFORE_CANONICAL,canonical_after_sha256:sha(canonical),module_sha256:sha(modTarget),production_changed:false,d1_migration:false,d1_write_delta:0,d1_read_delta_per_deep_check_max:1,worker_relative_import_added:false,telegram_send:false,telegram_recipients_changed:false,cloudflare_deploy:false,force_push:false,hard_gates_changed:false,strategy_weights_changed:false,thresholds_changed:false,automatic_execution:false,validated_signal:false,live_probability:false,new_sources_added:false,worker_copy_added:false,hot_cycle_external_request_delta:0};
