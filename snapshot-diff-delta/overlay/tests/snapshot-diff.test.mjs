...
test('worker contains one bounded previous-snapshot read without adding a relative Worker import',()=>{
 const w=fs.readFileSync(path.join(runtime,'src/worker.js'),'utf8');
 assert.match(w,/async function loadPreviousEvidenceSnapshotForCanonical/);
 assert.match(w,/FROM full_evidence_shadow_log/);
 assert.match(w,/ORDER BY\s+observed_ts DESC\s+LIMIT 1/);
 assert.match(w,/previous_snapshot_context:\s*previousSnapshotContext/);
 assert.doesNotMatch(w,/from\s+["']\.\/snapshot-diff\.mjs["']/);
