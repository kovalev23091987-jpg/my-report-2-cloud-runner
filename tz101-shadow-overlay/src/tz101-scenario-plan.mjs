...
export const TZ101_SCENARIO_PLAN_VERSION='tz101-scenario-plan-r7';
...
function verifyEntryAreaRule(rule,{decision,observedTs,currentPrice}={}) {
  if(!verifyImmutableEnvelope(rule,{schema:'tz101-entry-area-rule-v1',now:observedTs})||rule.status!=='CLOSED'||
    rule.calibration_status!=='VALIDATED_OUT_OF_SAMPLE'||rule.automatic_rule_promotion!==false||
    !text(rule.calibration_receipt_id)||!/^[0-9a-f]{16,64}$/.test(String(rule.calibration_dataset_digest||''))||rule.contract_code!==decision.contract_code||
...
export function buildTz101ScenarioPlan({
  ...
  const area=verifyEntryAreaRule(entryAreaRule,{decision,observedTs,currentPrice:plan.entry.reference_price});
  ...
  return {
    ...
    reasons,entry_area_rule_receipt_id:area?.receipt_id??null,automatic_trade:false,
