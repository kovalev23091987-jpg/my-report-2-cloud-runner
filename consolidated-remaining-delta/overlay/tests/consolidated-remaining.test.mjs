...
const [{buildProtectiveAssetFilter,MONTHLY_COLLAPSE_THRESHOLD_PCT},{buildEvidenceDomainContract},{fetchBoundedOfficialEvents},{computePostEventOutcome,OUTCOME_HORIZONS,SUPPLEMENTAL_OUTCOME_HORIZONS}]=await Promise.all([
...
test('R051 adds 15m/30m measurement support without changing six persisted horizons',()=>{
 assert.deepEqual(Object.keys(OUTCOME_HORIZONS),['1h','4h','12h','24h','3d','7d']);
 assert.equal(SUPPLEMENTAL_OUTCOME_HORIZONS['15m'],15*60000);
 assert.equal(SUPPLEMENTAL_OUTCOME_HORIZONS['30m'],30*60000);
