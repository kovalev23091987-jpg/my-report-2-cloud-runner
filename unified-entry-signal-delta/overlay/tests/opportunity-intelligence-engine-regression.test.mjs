...
test("integrated Opportunity event exposes exact closed 1m/3m/5m decomposition before early classification", () => {
  const oneMinute = minuteHistory();
  const now = oneMinute.at(-1).ts + MINUTE;
  const hourly = aggregateCandles(oneMinute, HOUR, MINUTE).candles;
  const analysis = buildOpportunityShadowAnalysis({
    ...
    primary: {
      ...
      four_hour: [],
      one_day: [],
      oi_contracts_hourly: hourly.map((row, i) => ({ ts: row.ts, volume: i < 10 ? 100 : i === 10 ? 90 : 96 })),
      windows: {},
    },
    external_hourly: { BYBIT_PERP: hourlyVenue(), OKX_SPOT: hourlyVenue(), BTC_SPOT: hourlyVenue(14, 12, 1), ETH_SPOT: hourlyVenue(14, 12, 1) },
  });
  const event = analysis.events.find((row) => row.timeframe === "15m");
  assert.ok(event, "15m event must exist in deterministic anomaly fixture");
  assert.equal(event.minute_decomposition?.status, "CLOSED");
  assert.equal(event.minute_decomposition?.classification_allowed, true);
  assert.equal(event.drilldown?.one_minute_bars, 15);
  assert.equal(event.drilldown?.three_minute_bars, 5);
  assert.equal(event.drilldown?.five_minute_bars, 3);
  assert.ok(event.early_anomaly_classification?.accumulation);
  assert.ok(event.early_anomaly_classification?.distribution);
  assert.ok(event.early_anomaly_classification?.two_sided_transfer);
  assert.ok(event.early_anomaly_classification?.liquidation_futures_noise);
});
