...
export const FREE_SOURCE_LIVE_PROBE_VERSION='free-source-live-probe-v1-20260925';
...
async function request(name,url,{method='GET',body=null,headers={}}={}){
  ...
  try{
    const response=await fetch(url,{method,headers:{'user-agent':'my-report-2-free-source-candidate/1.0',...headers},body:body?JSON.stringify(body):undefined,signal:controller.signal});
...
function yesterday(offset=2){const d=new Date(Date.now()-offset*86400000);return new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate()));}
async function main(){
  const probes=[];const d=yesterday(2);
  for(const market of ['spot','usd_m_futures']){
    ...
    const p=await request(`Binance Public Data Archive ${market}`,plan.checksum_url);if(p.status==='CLOSED'){const c=parseChecksumText(p.body_preview);p.checksum_status=c.status;p.checksum_sha256=c.sha256??null;}probes.push(p);
  }
  ...
  probes.push(await request('DefiLlama','https://api.llama.fi/protocol/aave'));
  ...
  probes.push(await request('Coinbase Exchange','https://api.exchange.coinbase.com/products/ONG-USDT'));
  ...
  const normalized=probes.map(p=>({source:p.source,status:p.status,http_status:p.http_status,latency_ms:p.latency_ms??null,bytes:p.bytes??null,checksum_status:p.checksum_status??null,error:p.error??null}));
