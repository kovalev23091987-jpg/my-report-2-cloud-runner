...
| Binance Public Data Archive | CANDIDATE | `data.binance.vision` archive plan for trades/aggTrades/klines | Spot vs USD-M split; sec/ms/us normalization; checksums | Maintenance artifacts/aggregates only; no raw trade D1 dump | Backcheck/history lane → later features/replay | No direct hot-cycle decision yet | Candidate CI checksum/download proof + source→feature→priority proof |
...
| DEX Screener | NOT_CONFIGURED | Public API documented; no current production consumer proof found | New chain+pool identity bridge prepared | Candidate probe/normalized pool observation only | Candidate advisory DEX context | None in production yet | Live probe + factual storage + existing analytical consumer proof |
| GeckoTerminal | NOT_CONFIGURED | Public API documented; no current production consumer proof found | Same pool dedupe with DEX Screener | Candidate probe only | Candidate advisory DEX context | None in production yet | Same as above; keyless not production-critical/high-frequency |
| DefiLlama | NOT_CONFIGURED | Free APIs/SDK support TVL/prices and available protocol/network context | Protocol/network identity | Candidate advisory normalization only | Supporting context | None until consumer proven | Live method + storage/consumer proof; unlock/emissions Pro not claimed free |
...
| GoPlus Security | BLOCKED | Candidate adapter prepared; current reference shows Authorization for token security | Exact chain+contract/mint; UNKNOWN/UNSUPPORTED != SAFE | No live receipt without approved auth | Supporting Risk only | No new Hard Gate; report may show verified risk/gap only after receipt | Owner-approved free account/auth secret if required; candidate no-auth probe may only clarify behavior |
