...
- `free-source-live-probe`: maximum 12 public calls, timeout 8s each, no secrets, D1, Telegram or trading.
...
- GoPlus: documentation states a free rate limit around 30 calls/min; current token-security reference shows Authorization, so live use remains blocked until authorized if required.
