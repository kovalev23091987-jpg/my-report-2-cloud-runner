export const CAPABILITY_REGISTRY_VERSION='capability-registry-v3-free-sources-20260925';
...
const ORDER=Object.freeze({
  ...
  supporting_risk:['GoPlus'],
  onchain:['Alchemy','Solana Public RPC'],
  protocol_context:['DefiLlama','DEX Screener','GeckoTerminal','CoinGecko'],
});
...
export function sourceExhaustion({metric,attempts=[],capabilities=[],now=Date.now()}={}){
  ...
  const statuses=trail.filter(x=>x.active).map(x=>x.status);
  let terminal=exhausted?'SOURCE_EXHAUSTED':'SOURCE_TOOL_UNAVAILABLE_THIS_RUN';
  if(statuses.length&&statuses.every(x=>x==='NOT_APPLICABLE'))terminal='NOT_APPLICABLE';
  else if(statuses.includes('BUDGET_EXHAUSTED'))terminal='BUDGET_EXHAUSTED';
  else if(statuses.length&&statuses.every(x=>['NOT_CONFIGURED','UNSUPPORTED'].includes(x)))terminal=statuses.every(x=>x==='UNSUPPORTED')?'UNSUPPORTED':'NOT_CONFIGURED';
  return {status:terminal,source:null,value:null,timestamp:null,trail,selection:{dynamic:capabilities.length>0,metric},missing_is_zero:false,stale_is_current:false,quota_exhausted_is_data_absent:false};
}
...
export function resolveCrossVenueConflict({metric,rows=[],tolerance_pct=5}={}){
  ...
  if(valid.length===2&&compatiblePairs.length===0)return {status:'CONFLICT',reason_code:'CROSS_VENUE_DIVERGENCE',metric,consensus:null,spread_pct:spreadPct,rows:valid,averaged:false,third_source_required:true};
  ...
  return {status:'CONFLICT',reason_code:'CROSS_VENUE_DIVERGENCE',metric,consensus:null,spread_pct:spreadPct,rows:valid,averaged:false,third_source_required:valid.length<3};
}
