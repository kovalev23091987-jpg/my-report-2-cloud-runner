...
export const SOURCE_REGISTRY_VERSION='source-registry-v1-free-sources-20260925';
...
const known=Object.freeze([
 ...
 {id:'DEX Screener',status:'NOT_CONFIGURED',scope:'DEX_POOL_CONTEXT',key_required:false},
 {id:'GeckoTerminal',status:'NOT_CONFIGURED',scope:'DEX_POOL_CONTEXT_NON_CRITICAL',key_required:false},
 {id:'DefiLlama',status:'NOT_CONFIGURED',scope:'TVL_NETWORK_FEES_VOLUME_CONTEXT',key_required:false},
 ...
 {id:'GoPlus',status:'BLOCKED',scope:'SUPPORTING_RISK_ONLY',key_required:true,blocker:'CURRENT_REFERENCE_REQUIRES_AUTHORIZATION_OWNER_SECRET_NOT_APPROVED'},
...
export function buildSourceRegistry({public_evidence=null,smart_money_raw=null,probe_results=[],extra_receipts=[]}={}){
  ...
  const entries=known.map(base=>{
    ...
    return {...base,status,runtime_receipt_count:receipts.length,current_closed_receipt_count:current.length,runtime_current:current.length>0,probe_status:probe?.status??null,missing_is_zero:false,stale_is_current:false,unsupported_is_safe:false};
  });
...
export function buildFreeSourceRuntimeSummary({public_evidence=null,smart_money_raw=null,publication_shadow=null,canonical=null,probe_results=[],extra_receipts=[]}={}){
  ...
  return {version:'free-source-runtime-summary-v1-20260925',status:'CLOSED',registry,budget,entry_funnel:diagnostic,hot_cycle_external_request_delta:0,d1_write_delta:0,analysis_only:true,no_new_hard_gate:true,no_strategy_weight_change:true,continuous_collector_status:'PARTIAL_REALTIME_COVERAGE'};
}
