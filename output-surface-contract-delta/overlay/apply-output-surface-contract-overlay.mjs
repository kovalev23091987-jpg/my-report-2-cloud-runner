...
const MAN_HEADER_OLD=" const lines=['МОЙ ОТЧЁТ 2',`Снимок рынка: ${result.snapshot_time_utc}`,`Запуск: ${result.run_id}`,''];";
const MAN_HEADER_NEW=" const lines=['МОЙ ОТЧЁТ 2',`Снимок рынка: ${result.snapshot_time_utc}`,''];";
...
for(const [label,s,anchor] of [['manual version',m,MAN_V_OLD],['manual header',m,MAN_HEADER_OLD],['manual sources',m,MAN_SRC_OLD],['manual return',m,MAN_RETURN_OLD],['telegram version',t,TG_V_OLD],['telegram evidence',t,TG_EVID_OLD],['canonical version',c,CAN_V_OLD],['canonical import',c,CAN_IMPORT_OLD],['canonical return',c,CAN_RETURN_OLD]])if(!s.includes(anchor))throw new Error(`OUTPUT_SURFACE_PATCH_ANCHOR_MISSING:${label}`);
m=m.replace(MAN_V_OLD,MAN_V_NEW).replace(MAN_HEADER_OLD,MAN_HEADER_NEW).replace(MAN_SRC_OLD,MAN_SRC_NEW).replace(MAN_RETURN_OLD,MAN_RETURN_NEW);
