...
const requestedSince=Number(process.env.REPORT2_SELF_AUDIT_SINCE_TS||0);
const enforceNew=String(process.env.REPORT2_SELF_AUDIT_ENFORCE_NEW||'0')==='1';
const auditStart=enforceNew && Number.isFinite(requestedSince) && requestedSince>0
  ? Math.max(fresh,Math.trunc(requestedSince))
  : fresh;
